...
export function applyHeatmapColoring(rootGroup, connections, nodeValues, sensors, options) {
  ...
  rootGroup.traverse((mesh) => {
    ...
    if (meta.type === 'Connection') {
      ...
      const centerVal = nodeValues.get(data.id) ?? minVal;
      const neighbors = (neighborMap.get(data.id) || [])
        .map((c) => {
          ...
          const nearControl = isStart ? pts[0] : pts[pts.length - 1];
          const farControl = isStart ? pts[1] : pts[pts.length - 2];
          const dt = Math.max(Math.abs(farControl.t - nearControl.t), 1e-6);
          const lenSeg = Math.max(dt * totalLen, 1e-6);
          const slope = (farControl.v - nearControl.v) / lenSeg;
          const startVal = nearControl.v;
