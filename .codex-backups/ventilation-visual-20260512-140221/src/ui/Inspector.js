...
export class Inspector {
  ...
  renderSources(slot, node, preview) {
    ...
    Object.entries(node.params.sources || {}).forEach(([sourceKey, source]) => {
      ...
      header.innerHTML = `<strong>${sourceLabel(sourceKey)}</strong><small>Path or imported local file</small>`;
      ...
      fileInput.type = 'file';
      fileInput.accept = sourceAccept(sourceKey);
  ...
  renderRoleMapping(slot, node, contract, preview) {
    ...
    (contract?.roles || []).forEach((role) => {
      ...
      options.forEach((candidate) => {
        ...
        option.textContent = candidate || '(not assigned)';
        select.appendChild(option);
      });
      select.value = node.params.roleMapping?.[role.key] || role.defaultPath || '';
      select.addEventListener('change', () => {
        node.params.roleMapping = node.params.roleMapping || {};
        node.params.roleMapping[role.key] = select.value;
        this.refreshNode(node);
      });
