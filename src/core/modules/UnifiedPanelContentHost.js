...
function normalizeContent(content = {}) {
  ...
  const fill = ['scene', 'canvas', 'chart'].includes(profile);
  return {
    profile,
    padding: content.padding || (fill ? 'none' : 'compact'),
    overflow: content.overflow || (fill ? 'hidden' : 'auto')
  };
}

function removeLegacyChrome(element) {
  if (!element) return;
  const directChildren = [...element.children];
  directChildren.forEach((child) => {
    if (child.matches?.(
      '.panel-title, .panel-heading, .panel-header, .runtime-panel-title, .panel-collapse-toggle'
    )) child.remove();
  });
  element.querySelectorAll?.(':scope > .panel-shell-title, :scope > .floating-panel-title').forEach((node) => node.remove());
}
...
function emitResize(element, detail) {
  ...
  const targets = new Set([
    ...
    ...element.querySelectorAll(
      'canvas, svg, .chart-panel, .chart-host, .echarts-container, [data-panel-resize-target]'
    )
  ]);
  targets.forEach((target) => {
    target.dispatchEvent(new CustomEvent('minevis:panel-resize', { detail }));
  });
}

export class UnifiedPanelContentHost {
  ...
  update({ element = this.element, onResize = this.onResize, content = this.content } = {}) {
    ...
    this.onResize = onResize;
    this.content = normalizeContent(content);
    removeLegacyChrome(this.element);
...
export { normalizeContent as normalizeUnifiedPanelContent, removeLegacyChrome };
